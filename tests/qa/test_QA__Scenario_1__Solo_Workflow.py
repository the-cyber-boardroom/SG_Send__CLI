...
class Test_QA__Scenario_1:
    ...
    def test__4__add_files_and_commit(self, shared):
        ...
        # After commit: +5 data objects (2 blobs + 1 configs sub-tree + 1 root tree + 1 commit)
        # Implementation uses sub-tree model: each directory gets its own tree object.
        # configs/EC2.json lives in a separate configs/ sub-tree referenced from root.
        #   data/:    7 (init tree + init commit + 2 blobs + configs sub-tree + root tree + commit)
    ...
    def test__7__add_more_files_and_commit(self, shared):
        ...
        # After second commit: +4 data (1 blob + 1 configs sub-tree re-encrypted + 1 root tree + 1 commit) = 11 data total
        # The configs/ sub-tree is re-encrypted on every commit (new nonce → new object ID) even if unchanged.
        counts = count_bare_files(vault_dir)
